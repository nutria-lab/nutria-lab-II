...
vi.mock("../../../services/mealPlanService", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../../../services/mealPlanService")>();
  return {
    ...actual,
...
describe("MealPlanPage", () => {
  ...
  it("lets the user switch to another day", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Avena con frutos rojos"),
  ...
  it("opens the recipe detail for a meal without losing the selected day", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /Avena con frutos rojos/ }),
    );

    expect(await screen.findByText("Mezclar todo")).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: /24/ })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  });
  ...
  it("shows a retry option on error and recovers when the retry succeeds", async () => {
    ...
    vi.mocked(mealPlanService.getCurrentMealPlan).mockResolvedValueOnce(
      mealPlanFixture,
    );
