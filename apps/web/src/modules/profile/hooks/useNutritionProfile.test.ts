...
vi.mock('../../../services/nutritionProfileService', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../../services/nutritionProfileService')>();
  return {
    ...actual,
...
describe('useNutritionProfile', () => {
  ...
  it('treats a missing profile (404) as a fresh start, not a technical error', async () => {
    vi.mocked(nutritionProfileService.getProfile).mockRejectedValue(new NutritionProfileNotFoundError());
  ...
  it('sets an error state for a technical load failure', async () => {
    vi.mocked(nutritionProfileService.getProfile).mockRejectedValue(new Error('network error'));
  ...
  it('rejects saving with duplicate excluded ingredients before calling the API', async () => {
    ...
    await act(async () => {
      await result.current.save({ ...sampleProfile, excludedIngredients: ['NUTS', 'NUTS'] });
    });
  ...
  it('keeps the caller-provided state available after a failed save', async () => {
    vi.mocked(nutritionProfileService.getProfile).mockResolvedValue(sampleProfile);
    vi.mocked(nutritionProfileService.updateProfile).mockRejectedValueOnce(new Error('network error'));
