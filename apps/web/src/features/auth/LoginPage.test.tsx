...
describe("LoginPage", () => {
  it("renders the approved Spanish login form with native sign-in semantics", () => {
    ...
    expect(register.parentElement).toHaveTextContent(
      "¿No tenés cuenta? Registrate",
    );
    expect(
      screen.queryByText("Tu bienestar, a tu ritmo"),
    ...
    expect(
      screen.getByRole("button", { name: /iniciá sesión|continuar/i }),
    ).toBeVisible();
  });
  ...
  it("exposes Registrate as the styleable semantic link to registration", () => {
    ...
    expect(register.parentElement?.firstElementChild).toBe(register);
    expect(register).toHaveAttribute("href", "/register");
  });
  ...
  it("keeps desktop branding separate from the labelled login region", () => {
    ...
    expect(
      screen
        .getByText("Nutrirte bien empieza con elegir con intención.")
        .closest("aside"),
    ).toHaveAttribute("aria-hidden", "true");
    expect(document.querySelector("form")?.closest("section")).toHaveAttribute(
      "aria-labelledby",
      "login-title",
    );
  });
  ...
  it("renders loading as an accessible presentation state that prevents repeated submission", () => {
    ...
    expect(
      screen.getByRole("status", { name: /iniciando sesión/i }),
    ).toBeVisible();
  ...
  it("does not delegate invalid local values or a submit while loading", async () => {
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      "Ingresá tu correo electrónico. Ingresá tu contraseña.",
    );

    rerender(
      <MemoryRouter>
        <LoginPage status="loading" onSubmit={onSubmit} />
      </MemoryRouter>,
    );
    await user.click(
