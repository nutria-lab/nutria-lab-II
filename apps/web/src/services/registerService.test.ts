...
describe('registerService.register', () => {
  ...
  it.each([
    ...
    [400, 'validation'],
  ] as const)('normalizes HTTP %i as %s without exposing the backend body', async (status, kind) => {
  ...
  it('normalizes malformed success responses and non-Axios failures as unexpected', async () => {
    vi.mocked(apiClient.post).mockResolvedValueOnce({ data: { id: registeredUser.id } });
