...
**Restricción de esta sesión de diseño, para que quede trazada:** no hay acceso a Neon MCP (no autenticado) y no se toca la base de datos local de la usuaria. Este documento sigue el protocolo de dos commits de `.ai/database-and-migrations.md`: todo lo que sigue llega hasta "generar y revisar" el SQL de migración, nunca hasta "ejecutar". Ningún comando `prisma migrate dev`, `prisma migrate deploy`, `prisma db push` ni ninguna consulta contra Neon se ejecuta como parte de este ticket ni de las etapas que lo consuman, hasta que TL apruebe explícitamente el commit 2.
...
- **Discrepancia detectada, documentada para que TL la tenga en cuenta (no es parte del alcance de este ticket resolverla):** `.ai/architecture.md` dice "AI: OpenAI a través de un límite server-side", pero el código real de generación de planes usa `@google/generative-ai` (Gemini, modelo `gemini-1.5-flash`, versión de prompt `1.0.0`). El campo `provider`/`model` de `GenerationRun` es agnóstico a esto (son `String` libres), así que el diseño no depende de resolver esta discrepancia, pero vale la pena que quede señalada.
